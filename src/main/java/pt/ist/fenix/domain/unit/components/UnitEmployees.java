...
public class UnitEmployees extends UnitSiteComponent {
    ...
    public void handle(Page page, TemplateContext componentContext, TemplateContext globalContext) {
        ...
        globalContext.put("hasEmployeesNoArea", activeNonTeacherEmployeesStream(unit).filter(hasWorkingPlace.negate()).findAny().isPresent());
    ...
    private SortedMap<Unit, TreeSet<Employee>> nonTeacherEmployeesByWorkingPlace(Unit unit) {
        return activeNonTeacherEmployeesStream(unit).filter(hasWorkingPlace)
